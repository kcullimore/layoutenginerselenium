...
function getTextNodes(parent) {
  ...
  for (parent = parent.firstChild; parent; parent = parent.nextSibling) {
    ...
    if (parent.nodeType === Node.TEXT_NODE) textNodes.push(parent);
    else textNodes = textNodes.concat(getTextNodes(parent));
  }
  return textNodes;
}
...
function writeBox(node, index, parentName) {
  ...
  if (
    ...
    line =
      line +
      (style["font-weight"] == "bold" || style["font-weight"] > 500
        ? "TRUE"
        : "FALSE") +
      ",";
